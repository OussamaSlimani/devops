terraform {
  backend "s3" {
    bucket         = "custom-terraform-state-bucket-123-eee2199c"  # Replace with your S3 bucket name
    key            = "terraform-project/terraform.tfstate"         # Location of the state file in the bucket
    region         = "us-east-1"                                   # AWS region
    dynamodb_table = "custom-terraform-state-locks-123"            # Replace with your DynamoDB table name
...
resource "aws_s3_bucket_object" "private_key_object" {
  bucket = "custom-terraform-state-bucket-123-eee2199c"  # Reference the same S3 bucket used for state
